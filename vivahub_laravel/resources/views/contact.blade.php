...
<div class="relative w-full lg:w-[60%] flex items-center justify-center p-4 sm:p-8 lg:p-20 bg-gray-5 dark:bg-background-dark transition-colors duration-300">
...
<h2 class="text-gray-900 dark:text-white text-3xl font-bold mb-2 transition-colors">Send us a Message</h2>
                ...
                @if(session('success'))
                <div class="mb-6 p-4 rounded-lg bg-green-500/10 border border-green-500/20 text-green-700 dark:text-green-400 flex items-center gap-3">
                    <span class="material-symbols-outlined">check_circle</span>
                    <p class="text-sm font-medium">{{ session('success') }}</p>
                </div>
                @endif
                
                <form class="space-y-8" method="POST" action="{{ route('contact.submit') }}">
@csrf
