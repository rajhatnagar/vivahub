...
<head>
  ...
  <title>{{ $invitation->data['groom_name'] ?? 'Sagar' }} & {{ $invitation->data['bride_name'] ?? 'Dipika' }} | Wedding Invitation</title>
...
<body class="overflow-hidden selection:bg-rose-500 selection:text-white font-sans" id="main-body">
  ...
  <div id="start-overlay" class="fixed inset-0 z-[100] bg-midnight-950 flex flex-col items-center justify-center transition-opacity duration-1000">
    <div class="text-center animate-pulse-slow space-y-6">
        <h1 class="font-script text-6xl text-rose-300 drop-shadow-[0_0_15px_rgba(244,63,94,0.5)]">
            {{ $invitation->data['bride_name'] ?? 'Dipika' }} & {{ $invitation->data['groom_name'] ?? 'Sagar' }}
        </h1>
  ...
  <div class="relative h-[100dvh] w-full overflow-hidden">
    ...
    <div class="relative z-10 h-full w-full max-w-7xl mx-auto flex flex-col md:grid md:grid-cols-2 px-4 md:px-12 items-center justify-center">
      ...
      <div class="flex flex-col items-center w-full justify-center text-center h-full">
        <!-- Content Container -->
        <div class="flex flex-col items-center justify-center w-full gap-4">
          <p class="text-rose-100 font-serif text-xs md:text-sm tracking-[0.4em] uppercase mb-4 animate-fade-in-up">Together With Their Families</p>
          
          <h1 class="font-script text-[4rem] md:text-9xl text-gradient-rose leading-[1.1] md:leading-none mb-6 drop-shadow-2xl py-2 animate-fade-in-up">
            {{ $invitation->data['bride_name'] ?? 'Dipika' }} <br class="md:hidden" /> <span class="text-4xl md:text-6xl text-white/50 font-serif my-2 block md:inline">&</span> <br class="md:hidden" /> {{ $invitation->data['groom_name'] ?? 'Sagar' }}
          </h1>
          
          <div class="w-full max-w-xs h-px bg-gradient-to-r from-transparent via-rose-500/50 to-transparent my-6"></div>
          
          <div class="flex flex-col md:flex-row items-center gap-4 md:gap-8 text-rose-200 animate-fade-in-up delay-100">
             <div class="flex items-center gap-2 bg-midnight-900/50 px-4 py-2 rounded-full backdrop-blur-sm border border-rose-500/20">
                <i data-lucide="calendar" class="w-4 h-4 text-rose-400"></i>
                <span class="font-serif text-lg tracking-wide">{{ \Carbon\Carbon::parse($invitation->data['date'] ?? '2026-12-12')->format('M d, Y') }}</span>
             </div>
             <div class="hidden md:block w-1 h-1 bg-rose-500 rounded-full"></div>
             <div class="flex items-center gap-2 bg-midnight-900/50 px-4 py-2 rounded-full backdrop-blur-sm border border-rose-500/20">
                <i data-lucide="map-pin" class="w-4 h-4 text-rose-400"></i>
                <span class="font-serif text-lg tracking-wide">{{ $invitation->data['location_name'] ?? 'Udaipur' }}</span>
             </div>
          </div>
        </div>
  ...
  <!-- COUNTDOWN -->
  <div class="bg-midnight-950 py-20 relative overflow-hidden">
    <div class="max-w-4xl mx-auto px-4 flex flex-col items-center relative z-10">
      <h2 class="font-serif text-3xl md:text-5xl text-rose-100 font-light tracking-wider mb-12">
        <span class="text-rose-400 italic font-script pr-2">Save</span> The Date
      </h2>
      <div class="grid grid-cols-4 gap-2 md:flex md:justify-center md:gap-10 mb-12 w-full max-w-2xl" id="countdown-container">
        <div class="glass-card rounded-sm p-2 md:p-4 flex flex-col items-center justify-center group border-t-2 border-t-rose-500/50">
          <span id="days" class="font-serif text-2xl md:text-5xl text-rose-100 font-light group-hover:text-rose-400 transition-colors">00</span>
          <span class="text-[8px] md:text-xs uppercase tracking-widest text-rose-300/60 mt-1 md:mt-2">Days</span>
        </div>
        <div class="glass-card rounded-sm p-2 md:p-4 flex flex-col items-center justify-center group border-t-2 border-t-rose-500/50">
          <span id="hours" class="font-serif text-2xl md:text-5xl text-rose-100 font-light group-hover:text-rose-400 transition-colors">00</span>
          <span class="text-[8px] md:text-xs uppercase tracking-widest text-rose-300/60 mt-1 md:mt-2">Hrs</span>
        </div>
        <div class="glass-card rounded-sm p-2 md:p-4 flex flex-col items-center justify-center group border-t-2 border-t-rose-500/50">
          <span id="minutes" class="font-serif text-2xl md:text-5xl text-rose-100 font-light group-hover:text-rose-400 transition-colors">00</span>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-24 items-center">
        <div class="relative group mx-auto max-w-sm w-full">
          <div class="relative h-[450px] overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-700">
            <img src="{{ $invitation->data['bride_photo'] ?? 'https://images.unsplash.com/photo-1595526114035-0d45ed16cfbf?auto=format&fit=crop&q=80&w=800' }}" class="w-full h-full object-cover">
            <div class="absolute bottom-6 left-6">
                <h3 id="bride-name" class="font-script text-5xl text-rose-200 mb-1">{{ $invitation->data['bride_name'] ?? 'Dipika' }}</h3>
        ...
        <div class="relative group mx-auto max-w-sm w-full md:mt-24">
          <div class="relative h-[450px] overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-700">
            <img src="{{ $invitation->data['groom_photo'] ?? 'https://images.unsplash.com/photo-1610173824052-a56b3e71d378?auto=format&fit=crop&q=80&w=800' }}" class="w-full h-full object-cover">
            <div class="absolute bottom-6 right-6 text-right">
                <h3 id="groom-name" class="font-script text-5xl text-rose-200 mb-1">{{ $invitation->data['groom_name'] ?? 'Sagar' }}</h3>
  ...
  <section id="gallery" class="py-24 px-4 bg-midnight-900 relative overflow-hidden">
      ...
      <div class="max-w-6xl mx-auto relative z-10">
          ...
          <div class="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              <div class="group relative h-64 md:h-80 overflow-hidden rounded-lg glass-card cursor-pointer md:-mt-12">
                  <img src="https://images.unsplash.com/photo-1515934751635-c81c6bc9a2d8?auto=format&fit=crop&q=80" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                  <div class="absolute inset-0 bg-gradient-to-t from-midnight-950 via-transparent to-transparent opacity-60"></div>
              </div>
              <div class="group relative h-64 md:h-80 overflow-hidden rounded-lg glass-card cursor-pointer">
                  <img src="https://images.unsplash.com/photo-1544078751-58fee2d8a03b?auto=format&fit=crop&q=80" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                  <div class="absolute inset-0 bg-gradient-to-t from-midnight-950 via-transparent to-transparent opacity-60"></div>
              </div>
              <div class="group relative h-64 md:h-80 overflow-hidden rounded-lg glass-card cursor-pointer md:mt-8">
                  <img src="https://images.unsplash.com/photo-1522673607200-1645062cd958?auto=format&fit=crop&q=80" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100">
                  <div class="absolute inset-0 bg-gradient-to-t from-midnight-950 via-transparent to-transparent opacity-60"></div>
              </div>
          </div>
      </div>
  ...
  <!-- RSVP -->
  <section class="py-24 px-4 relative bg-[#050b1f]">
    <div class="max-w-2xl mx-auto relative z-10">
        <div class="glass-card border border-rose-500/30 p-1 md:p-2 rounded-xl">
            <div class="border border-rose-500/20 rounded-lg p-8 md:p-12 bg-midnight-900/80 backdrop-blur-md">
                ...
                <form id="rsvp-form" class="space-y-8" method="POST" action="{{ route('rsvp.submit') }}">
                    @csrf
